...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	if (morse_sta_configure_channelization(NULL, wpa_s->conf->country))
		return -1;
				...
				struct hostapd_config *conf)
{
	...
	if (ssid->mode == WPAS_MODE_IBSS) {
		...
		if (morse_is_multi_channelization_country(conf->op_country)) {
			...
			if (morse_get_channelization(wpa_s->ifname,
			    &driver_channelization_scheme)) {
				wpa_printf(MSG_ERROR, "%s: morse_get_channelization failed cc %c%c",
				     ...
				     const struct wpa_interface *iface)
{
	...
	if (!is_mesh_or_ibss &&
	    morse_sta_configure_channelization(iface->ifname, wpa_s->conf->country))
		return -1;
#endif /* CONFIG_IEEE80211AH */
