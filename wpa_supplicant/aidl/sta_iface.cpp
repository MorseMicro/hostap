...
#include <type_traits>
//static_assert(sizeof(struct wpa_supplicant) > 0, "wpa_supplicant is incomplete");
		...
		const std::vector<uint8_t> &privEcKey)
{
	...
	if (net_role != DppNetRole::AP &&
		...
		wpa_printf(MSG_ERROR,
			   "DPP: Error: Invalid network role specified: %d", static_cast<int>(net_role));
		return {std::vector<uint8_t>(),
			createStatus(SupplicantStatusCode::FAILURE_UNKNOWN)};
	}
	...
	default:
		wpa_printf(MSG_ERROR,
			   "DPP: Invalid or unsupported security AKM specified: %d", static_cast<int>(security_akm));
		return {std::vector<uint8_t>(),
			createStatus(SupplicantStatusCode::FAILURE_UNKNOWN)};
	}
...
ndk::ScopedAStatus StaIface::test_aidl(struct wpa_supplicant *wpa_s)
{
	return ndk::ScopedAStatus::ok();
...
wpa_supplicant *StaIface::retrieveIfacePtr()
{
	...
	wpa_s = wpa_supplicant_get_iface(wpa_global_, ifname_.c_str());
	return wpa_s;
}
}  // namespace supplicant
}  // namespace wifi
