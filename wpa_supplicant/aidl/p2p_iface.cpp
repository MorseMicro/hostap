	...
	const std::string& passphrase)
{
	...
	if (wpas_p2p_group_add_persistent(
		...
		CHANWIDTH_USE_HT, he, 0, NULL, 0, 0, is6GhzAllowed(wpa_s), 0, group_owner_bssid)) {
		ret = -1;
	}
	...
	bool persistent, uint32_t go_intent)
{
	...
	int new_pin = wpas_p2p_connect(
		...
		vht, CHANWIDTH_USE_HT, he, 0, nullptr, 0, is6GhzAllowed(wpa_s), false, 0, NULL);
	if (new_pin < 0) {
		return {"", createStatus(SupplicantStatusCode::FAILURE_UNKNOWN)};
	}
	std::string pin_ret;
	...
	bool persistent, int32_t persistent_network_id)
{
	...
	if (ssid == NULL) {
		...
		if (wpas_p2p_group_add_persistent(
			...
			CHANWIDTH_USE_HT, he, 0, NULL, 0, 0, is6GhzAllowed(wpa_s), 0, NULL)) {
			return createStatus(SupplicantStatusCode::FAILURE_NETWORK_UNKNOWN);
		} else {
			return ndk::ScopedAStatus::ok();
		}
