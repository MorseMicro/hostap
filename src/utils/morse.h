...
/**
 * morse_set_s1g_ht_chan_pairs() - Set the s1g->ht channelization pairs
 *
 * @cc: Country code, or NULL to set the default scheme
 */
 ...
 * Returns: 0 on success, else -1
 */
int morse_sta_configure_channelization(const char *ifname, char *country);
 ...
 */
enum s1g_op_class_type morse_s1g_op_class_valid(u8 s1g_op_class, const struct ah_class **s1g_class);
 ...
 * @param report_error  Report error logs
 */
...
/**
 * Configure the BSS color for the network.
 *
 * Handles the morse_cli calls to set the BSS color
 *
 * @param ifname The name of the interface (e.g. wlan0)
 * @param s1g_bss_color The BSS color to be assigned to the network
 *
 * @return 0 on success, else an error code
 */
...
/**
 * Set active scan dwell period (ms)
 *
 * @param ifname The name of the interface (e.g. wlan0)
 * @param dwell_ms Dwell period (ms)
 *
...
/**
 * morse_get_channelization() - Query channelization scheme to use
 *
 * @ifname: The name of the interface (e.g. wlan0)
 * @channelization: The channelization scheme in use
 *
 * Returns: 0 on success, else an error code.
 */
