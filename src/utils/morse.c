...
static bool morse_s1g_op_class_has_cc(const struct ah_class *class, const char *cc)
{
	...
	while (i < ARRAY_SIZE(class->cc_list) && class->cc_list[i][0]) {
...
int morse_ht_freq_to_s1g_chan(int ht_freq, char *country)
{
	...
	ht_chan = HT_FREQ_TO_HT_CHAN(ht_freq);
	...
	return morse_ht_chan_to_s1g_chan(ht_chan);
}
					     ...
					     struct hostapd_hw_modes *mode, int s1g_op_chan)
{
	...
	chan = hw_mode_get_channel(mode, ht20_prim_freq, 0, NULL);
	...
	if (conf->secondary_channel && conf->s1g_prim_chwidth == S1G_PRIM_CHWIDTH_2) {
		...
		chan = hw_mode_get_channel(mode, ht20_sec_freq, 0, NULL);
		if (!chan || (chan->flag & HOSTAPD_CHAN_DISABLED))
			return true;
	}
	return false;
