...
int morse_sta_configure_channelization(const char *ifname, char *country)
{
	...
	if (morse_get_channelization(ifname, &driver_channelization_scheme) &&
...
int morse_set_interface(struct hostapd_iface *iface)
{
	...
	if (morse_is_multi_channelization_country(iface->conf->op_country)) {
		if (morse_get_channelization(iface->conf->bss[0]->iface,
	...
	ret = morse_set_channel(iface->conf->bss[0]->iface, oper_freq, oper_chwidth,
				prim_chwidth, iface->conf->s1g_prim_1mhz_chan_index);
	if (ret) {
	...
	ret = morse_set_s1g_op_class(iface->conf->bss[0]->iface, op_chan_class->s1g_op_class,
				     prim_chan_class->global_op_class);
	if (ret) {
	...
	ret = morse_set_bss_color(iface->conf->bss[0]->iface, iface->conf->bss[0]->s1g_bss_color);
	if (ret) {
